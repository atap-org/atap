...
// SignTemplateProof signs the template (excluding the proof field) and sets the Proof fields.
// Called by the "via" entity when publishing a template.
func SignTemplateProof(tmpl *models.Template, privateKey ed25519.PrivateKey, keyID string) error {
	m, err := templateWithoutProof(tmpl)
	if err != nil {
		return fmt.Errorf("sign template proof: %w", err)
	}

	payload, err := crypto.CanonicalJSON(m)
	if err != nil {
		return fmt.Errorf("sign template proof: canonical JSON: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("sign template proof: create signer: %w", err)
	}

	jws, err := signer.Sign(payload)
	if err != nil {
		return fmt.Errorf("sign template proof: sign: %w", err)
	}

	compact, err := jws.CompactSerialize()
	if err != nil {
		return fmt.Errorf("sign template proof: compact serialize: %w", err)
	}

	// Detach payload
	jwsParts := strings.Split(compact, ".")
	if len(jwsParts) != 3 {
		return fmt.Errorf("sign template proof: unexpected compact JWS format")
	}
	detached := jwsParts[0] + ".." + jwsParts[2]
