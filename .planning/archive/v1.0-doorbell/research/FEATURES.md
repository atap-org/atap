...
| Inbound channels (webhook URLs) | Agents need to receive signals from external services (Stripe, GitHub). Unique webhook URL per service enables per-service revocation and audit trails. | Medium | `POST /v1/entities/{id}/channels` returns unique webhook URL. Labels, tags, expiration, rate limiting. |
